...
async def parse_page(url: str, title: str = 'Index of /',
    ...
    rsp = await CLIENT.request('GET', url)
    data = rsp.json()
    contents = []
    sorts = []
    for item in data:
        ...
        if item.get('type') == 'dir':
            href = name
            await parse_page(url + name, title + name, path + name, level + 1)
        else:
            href = item.get('url')
    ...
    await gen_html(path + 'index.html', title, contents, level)
...
async def gen_html(filename: str, title: str, contents: list, level: int):
    ...
    headers = [
        f'''<!DOCTYPE html>
<head>
  <title>Jedore Python Mirrors</title>
  <meta charset="UTF-8">
...
<header>
  <div class="nav-container">
    ...
    <nav>
      ...
      <a href="mailto:jedore@qq.com" title="Mail">
...
<footer>
  <div><strong>&copy; <span id="year"></span> Jedore. All Rights Reserved.</strong></div>
</footer>
<script>
